...
chrome.runtime.onMessage.addListener(function(request, sender, sendResponse) {
      //console.log( 'in listener recType=' + recType + ' pauseRecording=' + request.pauseRecording );
		...
		if(request.checkRecState=="what"){var curState=rec.state; sendResponse({state: curState, timer: x});}
});

function PrepareTabRecording(recVolume){
	recType="tab";
	volume=recVolume;
	chrome.tabCapture.capture({audio:false, video:true}, function(streamTab){
...
chrome.tabs.onActivated.addListener(function(e){
	...
	if(recType=="tab" && rec.state=="paused" && e.tabId==recordingTab){
		OkToResume=true;
		chrome.notifications.create("extfy1", {type:"basic", iconUrl:"ok.png", title:"ExtensionForYou", message:"You are now ready to resume recording.", requireInteraction:true});
	};
});
